...
SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。SFT / RL / OPD の更新軌跡を数式と図で追うなら、次の記事が補助になります。
...
応答に点数をつけ、その点が高くなるように方策を更新する系列です。人間のフィードバックからの強化学習 (Reinforcement Learning from Human Feedback; RLHF)[^christiano] は、人間の評価から報酬を作る枠組みです。PPO (Proximal Policy Optimization)[^ppo] は、その報酬で方策を更新する方法として置きます。
...
GRPO (Group Relative Policy Optimization)[^deepseekmath] は、同じ入力 $x$ に対して複数の応答を生成し、そのグループ内で相対的な利得を作ります。DeepSeekMath[^deepseekmath] では、GRPO を PPO の変種として導入し、価値推定器を使わずに数学推論を改善しています。DeepSeek-R1[^deepseek_r1] でも、この価値推定器を省く方策更新が使われています。
...
本記事では RLVR を報酬の作り方として扱います。ロールアウト中の修正や、報酬を返した後に応答をどう直すかは次の記事が詳しいです。

https://tech-blog.abeja.asia/entry/llm-rl-rollout-correction-202606

## 生成中の文脈で蒸留する

蒸留に基づく調整では、教師モデルや追加情報を条件にした分布を教師信号にします。検証可能な報酬だけでは、出力のどの位置を直すべきかまでは見えにくいです。そこで、生成中の文脈で教師分布を使う方法と、追加情報を使った自己蒸留を同じ流れで扱います。
...
本記事では OPD から自己蒸留へつながる部分だけに絞っています。OPSD の学習手順や直感的な読み方は、次の記事が追いやすいです。
...
手元のデータから逆引きするなら、最初の候補は次のように置けます。
...
本文で扱った手法を、系列と教師信号だけに絞って置き直します。

| 手法                                     | 系列         | 主な教師信号                 | 要点                                                     |
| ---------------------------------------- | ------------ | ---------------------------- | -------------------------------------------------------- |
| SFT[^instructgpt]                        | 模範応答型   | 人間や教師が書いた応答       | 良い応答をまねる                                         |
| RLHF[^christiano] / PPO[^ppo]            | 報酬型       | 人間評価から作った報酬       | 人間に好まれる応答に高い報酬を与えて伸ばす               |
| RLVR[^rlvr]                              | 報酬型       | 検証可能な報酬               | 自動検証できる正誤を報酬にする                           |
| GRPO[^deepseekmath]                      | 報酬型       | 同じ問い内の相対利得         | 複数応答を比べて方策を更新する                           |
| DPO[^dpo]                                | 選好型       | 勝ち応答と負け応答の比較     | 好ましい応答を、好ましくない応答より出しやすくする       |
| SDPO 区間型[^segment_sdpo]               | 選好型       | 会話や推論の一部分への選好   | 会話や推論の一部分に DPO をかける                        |
| OPD[^opd_gkd]                            | 蒸留型       | 教師モデルの出力分布         | 現在のモデルが実際に出した途中文脈で、教師モデルをまねる |
| OPSD[^opsd]                              | 自己蒸留型   | 追加情報を条件にした分布     | 訓練時だけ使える情報を通常時の方策へ移す                 |
| SDPO 自己蒸留型[^self_distillation_sdpo] | 自己蒸留型   | 実行結果や評価文を条件にした分布 | 評価後の情報を通常時の方策へ移す                         |

エージェントの失敗を直すときは、手法名より先に、どの出力へどの粒度の教師信号を返せるかを切り分けます。模範応答で形式を教えるのか、報酬で最終結果を押すのか、比較で好ましい応答へ寄せるのか、生成途中の分布まで戻すのか。そこが変わると、必要なデータも訓練の構成も変わります。
