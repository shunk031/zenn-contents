...
こんにちは [@shunk031](https://twitter.com/shunk031) です。最近は、GPT や Claude の API を呼び出すだけではない AI エージェントの構築に興味があります。API を叩くだけでも便利ですが、失敗した挙動をプロンプトで吸収するのか、データ、報酬、蒸留で直すのかまでは判断しにくいです。[^api_wrapper] 本記事では、既存の大規模言語モデル (Large Language Model; LLM) の事後学習 (post-training) を、失敗へ返せる教師信号の違いから整理します。
...
まず、手法を教師信号とデータ形式で分けます。模範応答、比較、報酬、教師分布のどれをモデルへ返すのかで整理します。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、本記事では区間型と自己蒸留型に分けます。
...
応答に点数をつけ、その点が高くなるように方策を更新する系列です。人間のフィードバックからの強化学習 (Reinforcement Learning from Human Feedback; RLHF)[^christiano] は、人間の評価から報酬を作る枠組みです。Proximal Policy Optimization (PPO)[^ppo] は、その報酬で方策を更新する方法として扱います。
