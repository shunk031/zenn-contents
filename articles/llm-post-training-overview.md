...
OpenAI や Anthropic が学習してくれた範囲で満足できるなら、それで十分です。実際、それだけで動くタスクも多いです。ただ、自分が自動化したいタスクは、既存モデルが最初からうまく解けるものばかりではありません。そこで、自前の事後学習 (post-training) によって、既存の大規模言語モデル (Large Language Model; LLM) から必要な振る舞いを引き出したくなります。[^api_wrapper]
