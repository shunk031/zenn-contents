...
既存 LLM をどう事後学習すれば、ツール利用、探索、検証、修正を挟んでタスクを進められるのか。必要なデータと訓練構成を取り違えないように、手法ごとに何を学習に使うのかを比べます。

以前のスライドでは、LLM エージェント全般と Agentic Reinforcement Learning (Agentic RL)[^agentic_rl_survey] をまとめました。Agentic RL は事後学習とも重なりますが、スライドではエージェントの方法論まで広めに扱っています。この本文は、既存 LLM の振る舞いを変えるためのデータと学習方法に寄せたメモです。
...
日々提案される論文名を追うだけだと、必要なデータと訓練構成を取り違えやすいです。入力と模範応答の組で学ぶ場合、勝ち応答と負け応答の組を使う場合、検証器の点数を報酬にする場合、教師モデルの分布に合わせる場合では、必要なモデルや更新ループが変わります。
...
AI エージェントの失敗を直すには、まず失敗をどの単位で記録できるかが効きます。応答全体を採点できるだけなのか、勝ち応答と負け応答を比べられるのか、生成途中の文脈ごとに教師分布を用意できるのかで、使える目的関数は変わります。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、この記事では区間型と自己蒸留型に分けます。
...
同じ失敗ログでも、どの形のデータへ変換できるかで目的関数は変わります。
...
SFT、RLHF、DPO、RLVR / GRPO、OPD / OPSD / SDPO という名前だけを並べても、実装で必要になるものは見えてきません。形式崩れなら入力と修正版応答の組、望ましくない応答なら比較ペア、最終答えの誤りなら検証器の報酬、途中の修正まで扱うなら教師分布が必要になります。ここが変わると、集めるログ、走らせるサンプリング、持つべきモデルが変わります。
