...
こんにちは [@shunk031](https://twitter.com/shunk031) です。最近は、GPT や Claude の API を呼び出すだけで終わらない AI エージェントの構築に興味があります。API は便利ですが、OpenAI や Anthropic が出してきたモデルをただ叩いて驚くだけだと、そこで止まってしまいます。[^api_wrapper] 既存の大規模言語モデル (Large Language Model; LLM) をどう事後学習 (post-training) すれば、ツール利用、探索、検証、修正を挟んでタスクを進められるのか。この記事では、モデルへ渡す教師信号の違いから事後学習の手法を比べます。

以前のスライドでは、LLM エージェント全般や Agentic Reinforcement Learning (Agentic RL)[^agentic_rl_survey] まで広く扱いました。その後、関心は既存 LLM の振る舞いを変える事後学習へ移りました。
...
日々提案される論文だけを追っても、手法間の関係は見えにくいです。模範応答を直接まねる手法、比較や報酬で応答全体を評価する手法、検証結果や教師分布で途中の振る舞いへ信号を入れる手法では、必要なデータと訓練構成が変わります。
...
AI エージェントの失敗を直すときは、信号の種類と粒度が訓練構成に効いてきます。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、この記事では区間型と自己蒸留型に分けます。

| 学習に使う信号                       | 主な手法                                      | 論点                         |
| ------------------------------------ | --------------------------------------------- | ---------------------------- |
| 入力と模範応答                       | SFT[^instructgpt]                             | 模範応答上の文脈で学ぶ       |
| 人間評価や報酬                       | RLHF[^christiano] / PPO[^ppo]                 | 報酬モデルや価値推定器が絡む |
| 勝ち応答と負け応答                   | DPO[^dpo] / IPO[^ipo] / ORPO[^orpo] / SimPO[^simpo] / KTO[^kto] | 比較データから直接調整する   |
| 検証器が返す正誤や点数               | RLVR[^rlvr] / GRPO[^deepseekmath]             | 最終結果への報酬が中心になる |
| 区間ごとの選好                       | 区間型 SDPO[^segment_sdpo]                    | 応答全体ではなく区間を見る   |
| 教師分布や追加情報を条件にした分布   | OPD[^opd_gkd] / OPSD[^opsd] / 自己蒸留型 SDPO[^self_distillation_sdpo] | 生成中の文脈に信号を入れる   |

SFT や DPO は、あらかじめ用意した模範応答や比較データから学びます。GRPO や OPD 系では、現在のモデルが生成した応答上で方策や分布を更新します。目的関数の更新方向まで追うなら、SFT / RL / OPD の比較がまとまっています。

https://nrehiew.github.io/blog/sft_rl_opd/

## 共通の記法

以降の式では、入力、応答、方策、報酬、選好ペアを次の記号で表します。
...
入力と模範応答の組があるなら、モデルにはその応答を高い確率で出すように学ばせます。採点や比較の前に、回答形式、文体、基本動作をデータで入れる段階です。
...
入力と模範応答の組があれば、指示形式、文体、出力フォーマット、基本的なタスク解法を SFT で学習できます。InstructGPT でも、人が書いた模範応答で教師ありファインチューニングした後に、人間の比較を使っています。

SFT の信号は、基本的に模範応答上の文脈に限られます。訓練時は、模範応答の途中まで $y^\ast_{<t}$ を条件に $y^\ast_t$ を予測します。モデル自身の生成が模範応答から外れた後の文脈には、この損失から直接の学習信号が入りません。
...
InstructGPT で使われたような RLHF 構成では、この報酬 $r(x, y)$ を人間の好みから作ります[^ziegler][^stiennon]。代表的には、次の構成を取ります。
...
ここで $A_t$ は、その行動が平均よりどれだけ報酬を増やしたかを表す値です。PPO は、報酬に効いた出力の確率を上げ、そうでない出力の確率を下げます。ただし、確率比をクリップして一度の更新量を抑えます。

応答の評価をスカラー報酬にできるなら、RLHF / PPO は生成した応答を採点し、その報酬を最大化する方策更新として書けます。ただし、応答を生成する方策モデルのほかに、基準モデル、報酬モデル、価値推定器が絡みます。この構成を軽くする選択肢として、比較データから直接方策を動かす DPO や、価値推定器を省く GRPO があります。
...
直接選好最適化 (Direct Preference Optimization; DPO)[^dpo] は、報酬モデルを明示的に作らず、PPO の更新ループも使わずに、比較データから直接モデルを調整する方法です。DPO の出発点は、RLHF でよく使う KL 正則化つきの目的関数です。
...
Group Relative Policy Optimization (GRPO)[^deepseekmath] は、同じ入力 $x$ に対して複数の応答を生成し、そのグループ内で相対的な利得を作ります。DeepSeekMath では、GRPO を PPO の変種として導入し、価値推定器を使わずに数学推論を改善しています。DeepSeek-R1 でも、この価値推定器を省く方策更新が使われています。
...
数学の最終答えが一致するか、コードがテストを通るか、という課題では、GRPO に渡す報酬を自動で作れます。ただし、この報酬は疎な信号です。最終結果に点はつけられても、出力のどの位置やどの推論ステップが効いたのかまでは分かりません。
...
知識蒸留[^distillation] では、教師モデルの出力分布を生徒モデルにまねさせます。LLM では、教師モデルが作った固定データを生徒モデルに学ばせる形もよく使われます。この場合、生徒自身が今生成する文脈からは外れるため、オフポリシー蒸留と読めます。この対比でいうオンポリシーは、「現在のモデルが実際に生成する分布に沿う」という意味です。オンポリシー蒸留 (On-Policy Distillation; OPD)[^opd_gkd] は、生徒モデル自身が現在の方策で生成した応答を使います。

$$
y \sim \pi_\theta(\cdot \mid x)
$$

生徒が自分で応答 $y$ を生成したあと、その途中文脈 $x, y_{<t}$ に対して教師が次のトークン分布を与えます。学習では、教師分布と生徒分布の KL 距離を最小化します。
...
OPSD のデータの流れでは、教師側と生徒側の条件づけを分ける必要があります。以下の解説では、この条件づけを図で分けています。
...
ここで $w_t$ は、どの位置を強く学ばせるかを決める重みです。実行結果や評価文を条件にした分布を教師にするため、単なる成功・失敗の報酬よりも、修正対象を位置ごとの信号として渡せます。

## 使い分けの目安

訓練データの形が変わると、必要なモデルや更新ループも変わります。入力と模範応答の組なら、報酬モデルを持たずに SFT の損失を書けます。勝ち応答と負け応答の組なら、DPO のように比較から方策を直接動かせます。最終答えを自動検証できるなら報酬を作れますが、GRPO のような方策更新を使うには、同じ問いに対する複数応答も必要になります。

| 手法                                     | 現在のモデルが生成した応答を使うか | 教師信号                         |       報酬モデル | 基準モデル | 合う場面                           | 注意点                     |
| ---------------------------------------- | ---------------------------------: | -------------------------------- | ---------------: | ---------: | ---------------------------------- | -------------------------- |
| SFT[^instructgpt]                        |                           使わない | 模範応答の各位置                 |             不要 |       不要 | 入力と模範応答の組がある           | 生成時のずれに弱い         |
| RLHF[^christiano] + PPO[^ppo]            |                               使う | 応答全体の報酬                   |             必要 |   よく使う | 人間評価を報酬として使う           | 構成が重い                 |
| DPO[^dpo]                                |                   基本的に使わない | 勝ち応答と負け応答の組           |             不要 |       必要 | 比較データがある                   | 比較データに依存           |
| RLVR[^rlvr]                              |                               使う | 検証可能な報酬                   | 不要な場合が多い |       任意 | 数学・コードの正誤を報酬にできる   | 報酬が応答全体への点になる |
| GRPO[^deepseekmath]                      |                               使う | 同じ問い内の相対利得             | 不要な場合が多い |   よく使う | 価値推定器が不要                   | 報酬設計は別に必要         |
| OPD[^opd_gkd]                            |                               使う | 教師モデルの出力分布             |             不要 |       任意 | 現在の生成文脈で分布を学べる       | 教師モデルの計算が重い     |
| OPSD[^opsd]                              |                               使う | 追加情報を条件にした自己分布     |             不要 |       任意 | 外部教師が不要                     | 追加情報の設計が重要       |
| 区間型 SDPO[^segment_sdpo]               |                         場合による | 区間ごとの選好                   |             不要 | 多くは必要 | 長い会話や多段推論に向く           | 区間の選び方が難しい       |
| 自己蒸留型 SDPO[^self_distillation_sdpo] |                               使う | 実行結果や評価文を条件にした分布 |             不要 |       任意 | 実行後の情報を教師分布に変えられる | 評価文や実行結果の質に依存 |

訓練に使う材料から逆引きすると、次の対応になります。

| 用意できるデータ                     | 対応する手法                                   | 理由                               |
| ------------------------------------ | ---------------------------------------------- | ---------------------------------- |
| 入力と模範応答                       | SFT[^instructgpt]                              | 形式や基本動作を直接教えられる     |
| 勝ち応答と負け応答                   | DPO 系[^dpo]                                   | 報酬モデルなしで選好を入れられる   |
...
実際の訓練では、1 つの手法だけで済まないこともあります。形式崩れには入力と修正版応答の組、望ましくない応答には比較データ、最終答えの誤りには検証器の報酬を作る。さらに、ツール実行後の修正や推論途中の脱線まで扱うなら、OPD / OPSD / 自己蒸留型 SDPO のように、途中の文脈へ教師分布を与える構成も必要になります。

## まとめ

GPT や Claude の API を呼ぶだけでも、動くものはできます。ただ、AI エージェントの失敗をもう一段深く直そうとすると話が変わります。出力形式の崩れを直したいのか、ツール実行後の修正まで学ばせたいのかで、作るデータも訓練の組み方も変わります。

そのとき、SFT、RLHF、DPO、RLVR / GRPO、OPD / OPSD / SDPO という名前の列挙から入ると、どの手法がどの失敗に効くのかを見落としやすいです。自分が次に実装や論文を読むときは、まず「この方法はモデルに何を見せて、どの粒度の振る舞いを変えようとしているのか」から読むことにします。

<!-- textlint-disable ja-technical-writing/sentence-length -->

[^api_wrapper]: OpenAI や Anthropic の新モデルを叩いて「すごい」と言っているだけのエーアイ驚き屋で終わるのは、個人的にはあまりおもしろくありません。便利な API を使うにしても、中身の見通しを持ったうえで、振る舞いをどう変えられるのかまで踏み込みたい、という話です。
