...
事後学習の手法は増えていますが、名前だけを追っても関係が見えにくいです。模範応答をまねるのか、人間評価や比較データを使うのか、検証できる正誤や生成途中の教師分布を使うのか。まず、モデルに何を教師信号として渡しているのかで見たほうが迷いにくいと考えています。
...
手法名だけだと追いにくいので、まず教師信号で分けます。模範応答、比較、報酬、教師分布のどれを渡すのかを見ます。
...
SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。SFT / RL / OPD の幾何的な位置づけや更新軌跡には深入りしないので、その観点は次の記事に譲ります。
...
この損失は「模範応答を、どれだけ高い確率で再現できるか」を見ています。学習では $\pi_\theta(y^\ast \mid x)$ を大きくします。
...
この記事では RLVR を報酬の作り方として扱います。ロールアウト中の修正や実装上の論点には深入りしないので、その話は次の記事に譲ります。
...
ここでは OPD から自己蒸留へつながる見方だけに絞っています。OPSD そのものの流れは次の記事に譲ります。
...
実装前にまず見るのは、手元で新しく作れる教師信号です。模範応答しかないのか、比較ペアを作れるのか、自動検証できるのか、生成途中の分布まで戻せるのか。そこから候補を絞ります。
...
手元のデータに寄せると、最初に検討する手法は次のように分かれます。
...
| 正解や手がかりなどの追加情報がある   | OPSD[^opsd]                                    | 追加情報を見た自分を蒸留できる     |
| 実行結果や評価文を作れる             | SDPO 自己蒸留型[^self_distillation_sdpo]       | 評価後の情報を密な信号にしやすい   |
| 複数ターンのどこが悪いか分かる       | 区間型 SDPO[^segment_sdpo]                     | 応答全体ではなく重要区間を直せる   |

実務では、どれか 1 つだけを使うとは限りません。SFT で最低限の形式を作り、DPO で選好を入れます。検証可能なタスクでは RLVR の形で報酬を作り、GRPO のような方策更新を使うことがあります。応答全体の正誤だけでは足りない場合に、OPD / OPSD / SDPO のような生成途中の信号を検討します。
...
数学やコードのように、答えが正しいか自動で判定できるなら、RLVR が向いています。たとえば $r(x, y) \in \{0, 1\}$ のような報酬を作りやすいからです。同じ問いに複数の応答を出して比べられるなら、GRPO のような group-relative advantage を使う更新も候補になります。実行エラー、判定理由、正解の推論過程、追加のヒントなどがある場合は、まず何を教師信号にできるかを分けます。教師モデルの分布なら OPD、追加情報を見た自分の分布なら OPSD、実行結果や評価文を見た分布なら SDPO 自己蒸留型です。

## まとめ

記事全体をまとめると、事後学習は次の系列に分けて読めます。
...
| OPSD[^opsd]                              | 自己蒸留型   | 追加情報を見た自分の分布     | 追加情報を見た自分を、通常の自分に蒸留する               |
| SDPO 自己蒸留型[^self_distillation_sdpo] | 自己蒸留型   | 実行結果や評価文を見た分布   | 実行結果や評価文を見た自分を、通常の自分に蒸留する       |

エージェントの失敗を直すときは、手法名より先に、どの出力へどの粒度の教師信号を返せるかを見ます。模範応答で形式を教えるのか、報酬で最終結果を押すのか、比較で好ましい応答へ寄せるのか、生成途中の分布まで戻すのか。そこが変わると、必要なデータも訓練の構成も変わります。
