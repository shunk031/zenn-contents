...
こんにちは [@shunk031](https://twitter.com/shunk031) です。最近は、GPT や Claude の API を呼び出すだけではない AI エージェントの構築に興味があります。外側のラッパーだけを触っていると、失敗した挙動をプロンプトで吸収するのか、データ、報酬、蒸留で直すのかを切り分けにくいです。[^api_wrapper] 本記事では、その判断の前提として、既存の大規模言語モデル (Large Language Model; LLM) の事後学習 (post-training) を教師信号の違いから整理します。
...
事後学習の手法は増えていますが、名前だけを追っても関係が見えにくいです。模範応答をまねるのか、人間評価や比較データを使うのか、検証できる正誤や生成途中の教師分布を使うのか。モデルに返す教師信号から考えたほうが、必要なデータや訓練構成を切り分けやすいと考えています。
...
期待する入力と模範応答の組を用意できるなら、実装は比較的単純です。ただし生成時には、模範応答ではなくモデル自身の出力履歴を条件に続きを出します。その文脈でどう振る舞うべきかまで教えるには、別の信号が欲しくなります。人間評価を報酬にすれば、人間が好む応答へ寄せられますが、報酬モデルや価値推定器まで持つと訓練の構成は重くなります。比較データや検証可能な報酬を用意できる場合は、この重さを避けたり、評価を自動化したりする別の選択肢ができます。

AI エージェント的なタスクでは、最終的な成功や失敗だけでは足りない場面も多いです。ツールを呼び、結果を見て、方針を直す途中で、どの文脈のどの出力を直すべきかを学びたくなります。そのため、応答全体への粗い信号だけでなく、生成中の文脈に沿った細かい信号や、追加情報を条件にした分布を通常時へ移す方法まで扱います。

## LLM 事後学習の全体像

本記事で扱う手法を、教師信号とデータ形式で並べます。模範応答、比較、報酬、教師分布のどれをモデルへ返すのかで分けます。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、本記事では区間型と自己蒸留型に分けます。
...
SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。模範応答、強化学習、オンポリシー蒸留の目的関数が更新方向にどう現れるかは、次の記事で詳しく整理されています。
...
弱点は、訓練時と生成時のずれです。記号で書くと、訓練時は模範応答の途中まで $y^\ast_{<t}$ を条件に $y^\ast_t$ を予測します。生成時はモデル自身の履歴を条件に次のトークンを選ぶため、模範応答から外れた後の文脈には信号が薄くなります。
...
RLHF / PPO の強みは、応答の良し悪しをスカラー報酬として扱える点です。一方で、応答を生成する方策モデルのほかに、基準モデル、報酬モデル、価値推定器を持つ構成になりがちです。この構成を軽くする選択肢として、比較データから直接方策を動かす DPO や、価値推定器を省く GRPO があります。
...
DPO の後は、比較データをどう使うか、基準モデルをどう扱うかという方向で手法が分かれます。報酬モデル、基準モデル、ペア比較のどれを軽くするかで見ると、各手法の違いを追いやすくなります。

Identity Preference Optimization (IPO)[^ipo] は、DPO の近似がどの条件で崩れやすいかを、より一般的な選好学習の枠組みから見直します。DPO では、次の差を大きくし続けやすいという見方ができます。
...
本記事では RLVR を報酬の作り方に絞ります。ロールアウト中の失敗検出や補正ループは範囲外にし、実装寄りの話は次の記事に譲ります。

https://tech-blog.abeja.asia/entry/llm-rl-rollout-correction-202606

## 生成中の文脈で蒸留する

検証可能な報酬は、応答全体を押し上げる信号になりやすいです。この疎さを補う方法として、生成中の各文脈に教師分布を返す蒸留系の手法があります。ここからは、教師モデルの分布を使う方法と、追加情報を使った自己蒸留を並べます。
...
教師側は追加情報 $z$ を見られますが、生徒側は見られません。そのうえで、生徒が実際に生成した文脈上の出力分布を近づけます[^opsd]。訓練時だけ使える追加情報を条件にした分布を教師にし、推論時と同じく $z$ を見ない分布へ近づける、という形です。
...
ここで $\mathrm{sg}$ は勾配を止める操作です。教師側は更新せず、生徒側だけを教師に近づけます。数学やコードのように、「解いた後なら、どこが良かったか分かる」課題と相性がよいです。

本記事では OPD から自己蒸留へつながる部分だけに絞っています。OPSD のデータの流れや、教師側と生徒側で条件づけがどう変わるかは、次の記事の図解で追えます。
...
実務では、どれか 1 つだけを使うとは限りません。SFT で最低限の形式を作り、DPO で選好を入れることもあります。検証可能なタスクでは RLVR の形で報酬を作り、GRPO のような方策更新を使うことがあります。応答全体の正誤だけでは足りない場合に、OPD / OPSD / SDPO のような生成途中の信号を検討します。

## まとめ

最後に残る判断は、手法名ではなく教師信号の粒度です。形式や基本動作を教えるなら SFT、好ましさを入れるなら DPO や RLHF、正誤を返せるなら RLVR や GRPO、途中の文脈まで直したいなら OPD / OPSD / SDPO を検討します。エージェントの失敗を直すときも、どの出力へどの粒度の教師信号を返せるかを先に切り分けます。
