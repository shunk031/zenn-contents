...
背景として、LLM エージェント全般と Agentic Reinforcement Learning (Agentic RL)[^agentic_rl_survey] は以前のスライドにまとめています。この記事では、その続きとして事後学習を扱います。
...
事後学習の手法は増えていますが、名前だけを追っても関係が見えにくいです。自分がまず見たいのは、学習時に何を教師信号として使うかです。模範応答を直接まねるのか、比較や報酬で応答全体を評価するのか、検証結果や教師分布でもう少し細かく教えるのか。そこから逆算すると、必要なデータや訓練構成を切り分けやすくなります。
...
この記事では、各手法が何を教師信号にするかを基準にします。人間が書いた模範応答、応答同士の比較、応答全体への報酬、テスト結果や最終答えの正誤、教師モデルの出力分布の違いです。

期待する入力と模範応答の組を用意できるなら、まず模範応答をまねさせます。ただし生成時には、モデル自身の出力履歴を条件に続きを出します。模範応答から外れた後の文脈まで直したいなら、比較、報酬、検証結果、教師分布のような別の信号が必要になります。

AI エージェント的なタスクでは、ツールを呼び、結果を見て、方針を直す途中で失敗が起きます。最終的な成功/失敗だけで学ぶのか、途中の文脈に教師分布を与えるのかで、学習したい振る舞いは変わります。この記事では、その違いが表れる範囲を対象にします。

## LLM 事後学習の全体像

まず、手法を教師信号とデータ形式で分けます。模範応答、比較、報酬、教師分布のどれを使うかで整理します。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、本記事では区間型と自己蒸留型に分けます。
...
SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。目的関数の更新方向まで追う場合は、以下の記事が詳しいです。
...
ここでの RLVR は、検証可能な報酬をどう作るかの話です。ロールアウト中の失敗検出や補正ループは、以下の記事で実装寄りに扱われています。

https://tech-blog.abeja.asia/entry/llm-rl-rollout-correction-202606

## 生成中の文脈で蒸留する

検証可能な報酬は、応答全体を押し上げる信号になりやすいです。この疎さを補う方法として、生成中の各文脈に教師分布を与える蒸留系の手法があります。以下では、教師モデルの分布を使う方法と、追加情報を使った自己蒸留を分けます。
...
ここでは、OPD から自己蒸留へつながる部分を押さえます。OPSD で教師側と生徒側の条件づけがどう変わるか、データをどう流すかは、以下の記事の主題です。
...
最後は、失敗を観測できる単位から考えます。応答全体しか見えないなら、模範応答、比較、報酬、検証可能な正誤を使う手法が候補になります。途中の文脈や実行後の情報まで見えるなら、教師分布や自己蒸留でより細かい信号にできます。エージェントの失敗を直すときも、まず「どの出力に、どの粒度の教師信号を用意できるか」を決めるところから始めます。
