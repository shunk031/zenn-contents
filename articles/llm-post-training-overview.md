...
事後学習の手法は増えていますが、名前だけを追っても関係が見えにくいです。模範応答を直接まねる手法、比較や報酬で応答全体を評価する手法、検証結果や教師分布でより細かく教える手法では、必要なデータと訓練構成が変わります。
...
教師信号には、人間が書いた模範応答、応答同士の比較、応答全体への報酬、テスト結果や最終答えの正誤、教師モデルの出力分布などがあります。どの信号を使えるかで、手法の選び方は変わります。

AI エージェント的なタスクでは、ツールを呼び、結果を見て、方針を直す途中で失敗が起きます。最終的な成功/失敗だけを使う場合と、途中の文脈に教師分布を与える場合では、学習したい振る舞いも変わります。

## LLM 事後学習の全体像

次の表では、手法を教師信号とデータ形式で分けます。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、本記事では区間型と自己蒸留型に分けます。
...
OPSD の細かいデータの流れは、教師側と生徒側の条件づけを分けて読むと追いやすいです。この点は以下の記事で図解されています。
