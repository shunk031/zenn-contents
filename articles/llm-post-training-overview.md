...
| 検証器が返す正誤や点数             | RLVR[^rlvr] / GRPO[^deepseekmath] / MAXRL[^maxrl]                      | 最終答えやテスト結果       |
...
検証器の報酬を使うとき、もう 1 つ気になるのは、そもそも何を最大化しているのかです。数学やコードのように最終的な正誤を二値で判定できるなら、入力 $x$ に対して正解を出す確率を $p_\theta^{\mathrm{pass}}(x)$ と書けます。通常の強化学習は、この正解確率の期待値を上げます。一方で、Maximum Likelihood Reinforcement Learning (MAXRL)[^maxrl] は、同じ設定を正解確率の対数尤度を最大化する問題として見ます。

$$
J_{\mathrm{ML}}(\theta) = \mathbb{E}_{x \sim \rho} \left[ \log p_\theta^{\mathrm{pass}}(x) \right]
$$

この 2 つは、同じ検証器を使っていても更新の重みが違います。期待報酬の勾配は $\nabla_\theta p_\theta^{\mathrm{pass}}(x)$ ですが、対数尤度の勾配は次のように、正解確率が低い入力ほど大きく重み付けします。

<!-- textlint-disable -->

$$
\nabla_\theta \log p_\theta^{\mathrm{pass}}(x) =
\frac{1}{p_\theta^{\mathrm{pass}}(x)} \nabla_\theta p_\theta^{\mathrm{pass}}(x)
$$

<!-- textlint-enable -->

つまり MAXRL は、簡単に正解できる問題よりも、たまにしか正解しない難しい問題へ強い学習信号を置く目的関数として読めます。

実装上の差分は小さく、GRPO が報酬を標準偏差で正規化するのに対し、MAXRL は平均報酬で正規化します。成功サンプルが 1 つもない入力では更新できませんが、平均報酬が低い、つまり正解しにくい入力ほど 1 回の成功が強く効きます。論文では、ロールアウト数を増やすほど MAXRL の目的関数が最大尤度に近づく、と説明されています。

GRPO と MAXRL は、どちらも検証器が返す最終正誤を方策更新に使えます。違いは、同じ報酬をグループ内の相対利得として扱うのか、正解確率の対数尤度に近い重みとして扱うのかです。ただし、どちらの場合も報酬は最終結果への疎な信号です。テストが落ちた事実は報酬にできますが、どの推論ステップで戻るべきだったかまでは、この報酬だけでは残りません。
...
| MAXRL[^maxrl]                            |                               使う | 成功確率の対数尤度に近い重み     | 不要な場合が多い |   よく使う | 正誤を自動検証でき、複数ロールアウトを出せる | 成功サンプルがない入力は更新できない     |
...
| 難しい問題の成功を強く重み付けしたい | MAXRL[^maxrl]                            | 正解確率の低い入力を重く扱える   |
...
LLM の事後学習は、教師信号の形から見ると整理しやすいです。模範応答を使う SFT、選好ペアを使う DPO 系、報酬を使う RLHF、検証可能な正誤を使う RLVR / GRPO / MAXRL、生成途中の分布を合わせる OPD / OPSD / 自己蒸留系では、学習に戻せる情報の粒度が違います。
...
[^maxrl]:
    Fahim Tajwar et al. "Maximum Likelihood Reinforcement Learning."
    arXiv:2602.02710. DOI: 10.48550/arXiv.2602.02710. [https://arxiv.org/abs/2602.02710](https://arxiv.org/abs/2602.02710)
