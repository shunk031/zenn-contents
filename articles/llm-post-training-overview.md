...
OpenAI や Anthropic が学習してくれた範囲で満足できるなら、それで十分です。実際、それだけで動くタスクも多いです。ただ、自分が自動化したいタスクは、既存モデルが最初からうまく解けるものばかりではありません。たとえば、コード実行でテストが落ちているのにログを読まずに成功扱いする。検索結果に反証があるのに、最初の仮説のまま回答する。人間なら手順を教えればできるのに、モデルにはその振る舞いがまだ入っていない。そういう場面で、自前の事後学習 (post-training) によって、既存の大規模言語モデル (Large Language Model; LLM) から必要な振る舞いを引き出したくなります。[^api_wrapper]

既存 LLM をどう事後学習すれば、ツール利用、探索、検証、修正を挟んでタスクを進められるのか。この記事では、モデルへ渡す教師信号の違いから事後学習の手法を比べます。
...
日々提案される論文だけを追っても、手法間の関係は見えにくいです。自分はまず、手法名ではなく信号が入る場所で分類して読みます。模範応答を直接まねる手法、比較や報酬で応答全体を評価する手法、検証結果や教師分布で途中の振る舞いへ信号を入れる手法では、必要なデータと訓練構成が変わります。
...
AI エージェントの失敗を直すときは、失敗した出力のどこに信号を入れられるかが訓練構成に効いてきます。応答全体を採点するのか、勝ち負けを比べるのか、生成途中の文脈に分布を与えるのかで、直せる失敗は変わります。同じ事後学習でも、比較ペアを使う方法と検証器の報酬を使う方法では、拾える失敗の粒度が違います。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、この記事では区間型と自己蒸留型に分けます。

| 学習に使う信号                       | 主な手法                                      | 信号が入る場所                         |
| ------------------------------------ | --------------------------------------------- | -------------------------------------- |
| 入力と模範応答                       | SFT[^instructgpt]                             | 模範応答上の各位置                     |
| 人間評価や報酬                       | RLHF[^christiano] / PPO[^ppo]                 | 生成した応答全体                       |
| 勝ち応答と負け応答                   | DPO[^dpo] / IPO[^ipo] / ORPO[^orpo] / SimPO[^simpo] / KTO[^kto] | 応答ペアの相対差                       |
...
出力形式の崩れや基本動作の不足を直すなら、まず入力と模範応答の組を作ります。採点や比較の前に、回答形式、文体、基本動作をデータで入れる段階です。
...
SFT の信号は、基本的に模範応答上の文脈に限られます。訓練時は、模範応答の途中まで $y^\ast_{<t}$ を条件に $y^\ast_t$ を予測します。モデル自身の生成が模範応答から外れた後の文脈には、この損失から直接の学習信号が入りません。テストログを読まずに成功扱いするような失敗は、ここが SFT の限界になります。
...
テストや実行結果で正誤が返るなら、人間評価より先に報酬化を考えます。比較データだけでは、応答のどの部分が失敗したのかまでは分かりません。数学やコードのように自動検証できるタスクでは、最終答えやテスト結果を報酬にして、モデル自身の生成を更新に戻せます。
...
GRPO で欲しいのは、価値推定器ではなく、同じ問いに対する複数応答の相対差です。Group Relative Policy Optimization (GRPO)[^deepseekmath] は、同じ入力 $x$ に対して複数の応答を生成し、そのグループ内で相対的な利得を作ります。DeepSeekMath では、GRPO を PPO の変種として導入し、価値推定器を使わずに数学推論を改善しています。DeepSeek-R1 でも、この価値推定器を省く方策更新が使われています。
...
報酬が粗いなら、出力途中の分布を直接見たい。知識蒸留[^distillation] では、教師モデルの出力分布を生徒モデルにまねさせます。LLM では、教師モデルが作った固定データを生徒モデルに学ばせる形もよく使われます。この場合、生徒自身が今生成する文脈からは外れるため、オフポリシー蒸留と読めます。この対比でいうオンポリシーは、「現在のモデルが実際に生成する分布に沿う」という意味です。オンポリシー蒸留 (On-Policy Distillation; OPD)[^opd_gkd] は、生徒モデル自身が現在の方策で生成した応答を使います。
...
コード実行後のエラーや評価文まで残せるなら、それをただの成功/失敗で捨てるのはもったいないです。自己蒸留による方策最適化 (Self-Distillation Policy Optimization; SDPO)[^self_distillation_sdpo] は、実行結果、判定理由、評価文などを追加情報として使い、自己蒸留で密な学習信号へ変換する方法です。たとえばコード生成では、モデルがコードを書きます。そのコードを実行すると、実行エラーやテスト結果が返ってきます。この実行結果は、どの部分を直すべきかを推定する手がかりになります。しかし通常の強化学習では、最終的に成功なら 1、失敗なら 0 のような粗い報酬しか使わないことがあります。
...
GPT や Claude の API を呼ぶだけでも、動くものはできます。ただ、テストが落ちたログや検索結果の反証を読んだあとに方針を戻すところまで直したいなら、最終回答だけを見ていても足りません。出力形式の崩れを直したいのか、ツール実行後の修正まで学ばせたいのかで、作るデータも訓練の組み方も変わります。
...
自分は、事後学習の手法名を先に覚えるより、信号がどこに入るかを先に見るほうが実装に近いと考えています。次に論文を読むときは、最終報酬だけで足りるのか、生成途中の文脈まで信号を入れる必要があるのかを先に確認します。そこを外すと、名前だけ追っても実装には落ちません。
