...
こんにちは [@shunk031](https://twitter.com/shunk031) です。最近は、GPT や Claude の API を呼び出すだけではない AI エージェントの構築に興味があります。外側のラッパーだけを触っていると、失敗した挙動をプロンプトで吸収するのか、データ、報酬、蒸留で直すのかを切り分けにくいです。[^api_wrapper] 本記事では、既存の大規模言語モデル (Large Language Model; LLM) の事後学習 (post-training) を、失敗へ返せる教師信号の違いから整理します。

以下に、以前まとめたスライドを置いておきます。1 本目は LLM エージェント全般、2 本目は Agentic Reinforcement Learning (Agentic RL)[^agentic_rl_survey] の整理です。
...
この記事でそろえるのは、学習時にモデルへ返す信号です。モデルアーキテクチャやデータ収集基盤には踏み込みません。人間が書いた模範応答、応答同士の比較、応答全体への報酬、テスト結果や最終答えの正誤、教師モデルの出力分布に絞ります。

期待する入力と模範応答の組を用意できるなら、まず模範応答をまねさせます。ただし生成時には、モデル自身の出力履歴を条件に続きを出します。模範応答から外れた後の文脈まで直したいなら、比較、報酬、検証結果、教師分布のような別の信号が必要になります。

AI エージェント的なタスクでは、ツールを呼び、結果を見て、方針を直す途中で失敗が起きます。最終的な成功/失敗だけを返すのか、途中の文脈に教師分布を返すのかで、学習したい振る舞いは変わります。この記事では、その違いが表れる範囲を対象にします。

## LLM 事後学習の全体像

まず、手法を教師信号とデータ形式で置きます。模範応答、比較、報酬、教師分布のどれをモデルへ返すのかで分けます。なお、区間選好を使う手法と自己蒸留を使う手法には同じ略語が使われているため、本記事では区間型と自己蒸留型に分けます。
...
| RLVR[^rlvr]                                                           | 検証可能な報酬                         | 自動検証で正解する応答を増やす         | 問題、応答、正誤                 |
| GRPO[^deepseekmath]                                                   | 同じ問い内の相対利得                   | 複数応答を比べて方策を更新する         | 問題、複数応答、報酬             |
| OPD[^opd_gkd]                                                         | 教師モデルの分布                       | 現在の生成文脈で教師をまねる           | 生徒生成、教師分布               |
| OPSD[^opsd]                                                           | 追加情報を条件にした自己分布           | 追加情報つき分布を通常時へ蒸留する     | 問題、検証済み解法など           |
| SDPO 区間型[^segment_sdpo]                                            | 区間ごとの選好                         | 区間ごとに方策を直す                   | フィードバック、区間、複数ターン |
| SDPO 自己蒸留型[^self_distillation_sdpo]                              | 追加情報を条件にした分布               | 実行結果や評価文から密な信号を作る     | 実行結果、評価文                 |

SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。本記事では教師信号の違いに絞ります。模範応答、強化学習、オンポリシー蒸留の目的関数が更新方向にどう現れるかは、以下の記事が主題にしています。

https://nrehiew.github.io/blog/sft_rl_opd/

## 共通の記法

入力、応答、方策、報酬、選好ペアは、各手法で共通して出てきます。以降は、それらを同じ記号で書きます。
...
$\pi_\theta$ は、入力 $x$ に対して応答 $y$ を出す確率分布です。強化学習の言葉では、この確率分布を方策と呼びます。LLM では、ここまでの文脈から次に出すトークンを選ぶ確率分布として扱います。応答全体の確率は、各時点の出力確率の積として分解できます。
...
この損失は、模範応答をどれだけ高い確率で再現できるかを評価します。学習では $\pi_\theta(y^\ast \mid x)$ を大きくします。

模範応答を用意できるなら、SFT は最短の選択肢になります。指示形式、文体、出力フォーマット、基本的なタスク解法を同じデータで教えられます。InstructGPT[^instructgpt] でも、まず人が書いた模範応答で教師ありファインチューニングし、その後に人間の比較を使っています。

SFT だけでは、訓練時と生成時のずれが残ります。記号で書くと、訓練時は模範応答の途中まで $y^\ast_{<t}$ を条件に $y^\ast_t$ を予測します。生成時はモデル自身の履歴を条件に次のトークンを選ぶため、模範応答から外れた後の文脈には信号が薄くなります。

SFT は出力の各位置に細かい信号を与えられます。ただし、その信号は基本的に模範応答上の文脈に限られます。

## 人間評価を報酬にする

応答に点数をつけ、その点が高くなるように方策を更新する系列です。人間のフィードバックからの強化学習 (Reinforcement Learning from Human Feedback; RLHF)[^christiano] は、人間の評価から報酬を作る枠組みです。Proximal Policy Optimization (PPO)[^ppo] は、その報酬で方策を更新する方法として置きます。
...
強化学習で上げたいのは、モデルが自分で生成した応答の期待報酬です。ただし報酬だけを見ると、基準モデルから大きく外れた応答も選ばれやすくなります。LLM の RLHF では、期待報酬を上げつつ基準モデルからのずれを抑えるために、Kullback-Leibler (KL) 正則化を入れることが多いです。
...
ここで $A_t$ は、その行動が平均よりどれくらい良かったかを表す値です。利得差、または有利度と考えるとよいです。PPO は、良かった出力の確率を上げ、悪かった出力の確率を下げます。ただし、確率比をクリップして一度の更新量を抑えます。

応答の良し悪しをスカラー報酬にできるなら、RLHF / PPO はその報酬を直接上げにいけます。ただし、応答を生成する方策モデルのほかに、基準モデル、報酬モデル、価値推定器を持つ構成になりがちです。この構成を軽くする選択肢として、比較データから直接方策を動かす DPO や、価値推定器を省く GRPO があります。
...
DPO が上げたいのは、絶対的な報酬ではなく、基準モデルに対する勝ち応答と負け応答の差です。報酬モデルを別に持たず、訓練中のサンプリングも不要なので、PPO より扱いやすいです。
...
DPO は、「基準モデルと比べて、好ましい応答を出しやすくする」「好ましくない応答を出しにくくする」方法です。

DPO は、既存の比較データに強く依存します。比較ペアが粗い場合や、生成中のどこが悪かったのかを知りたい場合は、応答全体への信号だけでは足りません。長い推論では、途中の一手だけが原因で失敗することもあります。
...
Kahneman-Tversky 最適化 (Kahneman-Tversky Optimization; KTO)[^kto] は、ペア比較を作る負担を避けるために、望ましいかどうかの二値信号から学ぶ方向です。「この応答は望ましい」「この応答は望ましくない」という単独評価でも学びやすくするため、人間は損失に敏感で利益には鈍感、という行動経済学の考え方を目的関数に入れます。

このあたりは、何を省くかの違いとして読むと混乱しにくいです。報酬モデルを持つのか、基準モデルを残すのか、ペア比較を要求するのかを分けて考えます。

### 区間単位の直接選好最適化

区間単位の直接選好最適化 (Segment-Level Direct Preference Optimization; SDPO)[^segment_sdpo] は、会話や推論の一部分に DPO をかける方法です。長い会話では、応答全体を 1 つの選好ラベルで扱っても、改善すべき区間は分かりにくいです。そこで、重要な区間だけを取り出します。なお、SDPO という略語は自己蒸留系の論文でも使われるため、本記事では後の章で SDPO 自己蒸留型として分けます。
...
比較データだけでは、応答のどの部分が失敗したのかまでは分かりません。数学やコードのように自動検証できるタスクでは、最終答えやテスト結果を報酬にして、モデル自身の生成を更新に戻せます。
...
Group Relative Policy Optimization (GRPO)[^deepseekmath] は、同じ入力 $x$ に対して複数の応答を生成し、そのグループ内で相対的な利得を作ります。DeepSeekMath[^deepseekmath] では、GRPO を PPO の変種として導入し、価値推定器を使わずに数学推論を改善しています。DeepSeek-R1[^deepseek_r1] でも、この価値推定器を省く方策更新が使われています。
...
PPO[^ppo] は価値推定器を使って良さを見積もります。GRPO[^deepseekmath] はそこを省き、同じ問いに対する複数応答の相対比較から良さを推定します。

数学の最終答えが一致するか、コードがテストを通るか、という課題では、GRPO に渡す報酬を自動で作りやすいです。ただし、この報酬は疎になりやすいです。最終結果に点はつけられても、出力のどの位置やどの推論ステップが効いたのかまでは分かりにくいからです。

本記事では RLVR を報酬の作り方に絞ります。ロールアウト中の失敗検出や補正ループは範囲外です。Agentic RL の実装に寄せると、この部分は以下の記事の主題になります。

https://tech-blog.abeja.asia/entry/llm-rl-rollout-correction-202606

## 生成中の文脈で蒸留する

検証可能な報酬は、応答全体を押し上げる信号になりやすいです。この疎さを補う方法として、生成中の各文脈に教師分布を返す蒸留系の手法があります。以下では、教師モデルの分布を使う方法と、追加情報を使った自己蒸留を分けます。
...
生徒が自分で応答 $y$ を生成したあと、その途中文脈 $x, y_{<t}$ に対して教師が次に何を出しやすいかを確認します。学習では、教師分布と生徒分布の KL 距離を最小化します。
...
教師側は追加情報 $z$ を見られますが、生徒側は見られません。そのうえで、生徒が実際に生成した文脈上の出力分布を近づけます[^opsd]。訓練時だけ使える追加情報を条件にした分布を教師にし、推論時と同じく $z$ を見ない分布へ蒸留します。
...
ここで $\mathrm{sg}$ は勾配を止める操作です。教師側は更新せず、生徒側だけを教師に近づけます。数学やコードのように、「解いた後なら、どこが良かったか分かる」課題で使いやすい設定です。

本記事では OPD から自己蒸留へつながる部分だけに絞っています。OPSD で教師側と生徒側の条件づけがどう変わるか、データをどう流すかは、以下の記事の主題です。

https://note.com/kei_disign/n/nd60818508c1a

### 自己蒸留による方策最適化

自己蒸留による方策最適化 (Self-Distillation Policy Optimization; SDPO)[^self_distillation_sdpo] は、実行結果、判定理由、評価文などを追加情報として使い、自己蒸留で密な学習信号へ変換する方法です。たとえばコード生成では、モデルがコードを書きます。そのコードを実行すると、実行エラーやテスト結果が返ってきます。この実行結果は、どの部分を直すべきかを推定する手がかりになります。しかし通常の強化学習では、最終的に成功なら 1、失敗なら 0 のような粗い報酬しか使わないことがあります。
...
ここで $w_t$ は、どの位置を強く学ばせるかを決める重みです。実行結果や評価文を条件にした分布を教師にするため、単なる成功・失敗の報酬よりも、出力のどこやどの推論段階を直すべきかを学びやすいです。

## 使い分けの目安

実装前に確認したいのは、手元で新しく作れる教師信号です。模範応答、比較ペア、自動検証、生成途中の分布のどこまで用意できるかで、候補はかなり変わります。
...
| RLVR[^rlvr]                              |                               使う | 検証可能な報酬                   | 不要な場合が多い |       任意 | 数学・コードの正誤を使いやすい     | 報酬が疎になりやすい       |
| GRPO[^deepseekmath]                      |                               使う | 同じ問い内の相対利得             | 不要な場合が多い |   よく使う | 価値推定器が不要                   | 報酬設計は別に必要         |
| OPD[^opd_gkd]                            |                               使う | 教師モデルの出力分布             |             不要 |       任意 | 現在の生成に沿って細かく学べる     | 教師モデルの計算が重い     |
| OPSD[^opsd]                              |                               使う | 追加情報を条件にした自己分布     |             不要 |       任意 | 外部教師が不要                     | 追加情報の設計が重要       |
| SDPO 区間型[^segment_sdpo]               |                         場合による | 区間ごとの選好                   |             不要 | 多くは必要 | 長い会話や多段推論に向く           | 区間の選び方が難しい       |
| SDPO 自己蒸留型[^self_distillation_sdpo] |                               使う | 実行結果や評価文を条件にした分布 |             不要 |       任意 | 失敗原因を細かく学べる             | 評価文や実行結果の質に依存 |

手元のデータから逆引きすると、最初の候補はこうなります。
...
| 正誤を自動検証できる                 | RLVR[^rlvr]                                    | 検証可能な報酬を作りやすい         |
| 同じ問いに複数応答を生成できる       | GRPO[^deepseekmath]                            | 応答間の相対利得を作りやすい       |
| 教師モデルの分布を使える             | OPD[^opd_gkd]                                  | 生成中の文脈で教師分布を渡せる     |
| 正解や手がかりなどの追加情報がある   | OPSD[^opsd]                                    | 追加情報つき分布を通常時へ移せる   |
| 実行結果や評価文を作れる             | SDPO 自己蒸留型[^self_distillation_sdpo]       | 評価後の情報を密な信号にしやすい   |
| 複数ターンのどこが悪いか分かる       | 区間型 SDPO[^segment_sdpo]                     | 応答全体ではなく重要区間を直せる   |

実務では、どれか 1 つだけを選ぶより、どの段階の失敗を観測できるかで組み合わせます。形式が崩れるなら SFT、好ましさがずれるなら DPO や RLHF、最終答えを検証できるなら RLVR、同じ問いへの複数応答を比べられるなら GRPO が候補になります。途中の文脈まで観測できるなら、OPD / OPSD / SDPO のような生成途中の信号を検討します。

## まとめ

最後は、失敗を観測できる単位から考えます。応答全体しか見えないなら、模範応答、比較、報酬、検証可能な正誤を返す手法が候補になります。途中の文脈や実行後の情報まで見えるなら、教師分布や自己蒸留でより細かい信号にできます。エージェントの失敗を直すときも、まず「どの出力に、どの粒度の教師信号を返せるか」を決めるところから始めます。

<!-- textlint-disable ja-technical-writing/sentence-length -->

[^api_wrapper]: API 利用を否定したいわけではありません。便利だからこそ、その外側で何を足せば振る舞いが変わるのかを見ておきたい、という話です。
