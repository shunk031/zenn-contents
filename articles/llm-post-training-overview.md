...
こんにちは [@shunk031](https://twitter.com/shunk031) です。最近は、GPT や Claude の API を呼び出すだけではない AI エージェントの構築に興味があります。外側のラッパーだけを触っていると、失敗した挙動をプロンプトで吸収するのか、データ、報酬、蒸留で直すのかを切り分けにくいです。[^api_wrapper] そこで本記事では、既存の大規模言語モデル (Large Language Model; LLM) の事後学習 (post-training) を、手法名ではなく教師信号の違いから整理します。LLM エージェント全般は 1 本目のスライドで、Agentic Reinforcement Learning (Agentic RL)[^agentic_rl_survey] は 2 本目のスライドで扱っています。
...
本記事で扱う手法を、教師信号とデータ形式で並べます。模範応答、比較、報酬、教師分布のどれをモデルへ返すのかで分けます。
...
SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。更新軌跡を模範応答、強化学習、オンポリシー蒸留の目的関数から見比べるには、次の記事の整理が近いです。
...
InstructGPT 型、または PPO 型の RLHF では、この報酬 $r(x, y)$ を人間の好みから作ります[^ziegler][^stiennon]。代表的には、次の構成を取ります。
...
本記事では RLVR を報酬の作り方に絞ります。ロールアウト中の失敗検出や補正ループまで追う場合は、次の記事に具体例があります。

https://tech-blog.abeja.asia/entry/llm-rl-rollout-correction-202606

## 生成中の文脈で蒸留する

蒸留に基づく調整では、教師モデルや追加情報を条件にした分布を教師信号にします。検証可能な報酬だけでは、出力のどの位置を直すべきかまでは見えにくいです。ここからは、生成中の文脈で教師分布を使う方法と、追加情報を使った自己蒸留を並べます。
...
本記事では OPD から自己蒸留へつながる部分だけに絞っています。OPSD のデータの流れや教師側・生徒側の条件づけを追う場合は、次の記事の図解が近いです。
