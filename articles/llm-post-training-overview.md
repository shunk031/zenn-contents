...
SFT は固定データ、RLVR は報酬、OPD はオンポリシー蒸留として見る。この関係は次の記事でも整理されています。
...
応答に点数をつけ、その点が高くなるように方策を更新する系列です。ここでは、人間の評価から報酬を作る RLHF / PPO を見ます。
...
## 検証可能な報酬に基づく調整 (RLVR / GRPO)

数学やコードでは、最終答えやテスト結果を報酬にしやすいです。RLVR はこの設定を広く指し、GRPO はその方策更新の 1 つです。
...
## 蒸留に基づく調整 (OPD / OPSD / SDPO)

蒸留に基づく調整では、教師モデルや追加情報を見た自分の分布を教師信号にします。
...
SDPO という名前は、少なくとも 2 系統で使われています。同じ略語でも、論文によって見ている対象が違います。

- 区間単位の直接選好最適化 (Segment-Level Direct Preference Optimization; SDPO)[^segment_sdpo]: 複数ターン対話の重要な区間に絞って DPO をかける。
- 自己蒸留による方策最適化 (Self-Distillation Policy Optimization; SDPO)[^self_distillation_sdpo]: 実行結果、判定理由、評価文などを追加情報として使い、自己蒸留で密な学習信号へ変換する。

前者は区間ごとの選好、後者は追加情報を見た自分の分布を使います。タイトルと文脈を見て区別した方が安全です。
