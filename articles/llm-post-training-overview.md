...
こんにちは [@shunk031](https://twitter.com/shunk031) です。最近は、GPT や Claude の API を呼び出すだけではない AI エージェントの構築に興味があります。API を呼ぶだけでも便利ですが、エージェントとしてタスクを解かせるなら、モデルがどの信号で振る舞いを変えるのかまで見ておきたいです。[^api_wrapper] 本記事では、既存の大規模言語モデル (Large Language Model; LLM) をどう事後学習 (post-training) すれば、ツール利用、探索、検証、修正を挟みながらタスクを解けるようになるのかを整理します。以下に、LLM エージェントと Agentic RL について以前まとめたスライドも載せておきます。
...
事後学習の手法は増えていますが、名前だけを追っても関係が見えにくいです。模範応答をまねるのか、人間評価や比較データを使うのか、検証できる正誤や生成途中の教師分布を使うのか。自分はまず、モデルに何を教師信号として渡しているのかで見たほうが迷いにくいと考えています。
...
手法名だけだと追いにくいので、まず教師信号で分けます。模範応答を渡すのか、比較を渡すのか。報酬を作るのか、教師分布をまねさせるのか。そういう見方です。
...
SFT や DPO は、あらかじめ用意した模範応答や比較データから学ぶ色が強いです。一方で、GRPO や OPD 系は、現在のモデルが生成した応答を学習に戻します。この記事では教師信号の違いに絞りますが、SFT / RL / OPD の位置づけそのものは次の記事が追いやすいです。
...
ここでは RLVR を報酬の作り方として扱います。ロールアウト中の修正や実装上の論点は、次の記事のほうが詳しいです。
...
教師側は追加情報 $z$ を見られますが、生徒側は見られません。そのうえで、生徒が実際に生成した文脈上の出力分布を近づけます[^opsd]。訓練時だけ「答えや手がかりを見た自分」を作り、その分布を通常の自分へ移します。
...
ここで $\mathrm{sg}$ は勾配を止める操作です。教師側は更新せず、生徒側だけを教師に近づけます。「答えや手がかりを見た自分」が「答えや手がかりを見ていない自分」に教える方法、と読むと分かりやすいです。数学やコードのように、「解いた後なら、どこが良かったか分かる」課題と相性がよいです。

ここでは OPD から自己蒸留へつながる見方だけに絞っています。OPSD そのものの流れは、次の記事が追いやすいです。
...
ここで $w_t$ は、どの位置を強く学ばせるかを決める重みです。「失敗後の反省を見た自分」が「失敗前の自分」に教える方法、と読むと分かりやすいです。単なる成功・失敗の報酬よりも、出力のどこやどの推論段階を直すべきかを学びやすいです。

## 使い分けの目安

ここからは、手元で用意できる信号から選ぶ前提で整理します。最初の表では、教師信号と構成の重さを並べます。
...
実務では、どれか 1 つだけを使うとは限りません。SFT で最低限の形式を作り、DPO で選好を入れます。検証可能なタスクでは RLVR の形で報酬を作り、GRPO のような方策更新を使うことがあります。教師モデルの分布を使えるなら OPD、正解や手がかりを見た自己分布を使えるなら OPSD、実行結果や評価文を密な信号にしたいなら SDPO 自己蒸留型、と条件を分けて考えます。
...
事後学習は、何を教師信号として使うかの設計です。模範応答、選好、報酬、検証結果、追加情報を見た自己分布のどれを使うかで、必要なデータ、訓練の構成、得意なタスクが変わります。新しい手法を読むときも、まず「模範応答をまねるのか」「報酬で伸ばすのか」「比較から学ぶのか」「分布を蒸留するのか」を確認するところから始めるのがよさそうです。

<!-- textlint-disable ja-technical-writing/sentence-length -->

[^api_wrapper]: API 利用を否定したいわけではありません。ただ、外側のラッパーだけを触っていると、どの失敗をデータ、報酬、蒸留で直せるのか判断しづらい、という感覚があります。
